...
export default function SkillList({ skills }) {
  return <ul className="list-style-none text-center">
    {skills.map(({ name, value }) => (<li className="flex justify-center items-center my-2" key={name}>
      <span className="flex-grow-0 flex-shrink-0 basis-[35%] md:basis-[20%] font-semibold text-white-strong text-left pe-4">{name}</span>
      <div className="flex-grow h-1 bg-white-10">
        <div
